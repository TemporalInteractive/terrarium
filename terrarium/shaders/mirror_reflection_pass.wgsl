...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>,
    ...
    if (any(id >= constants.reflection_resolution)) { return; }

    let full_res_id: vec2<u32> = vec2<u32>(vec2<f32>(id) * (vec2<f32>(constants.resolution) / vec2<f32>(constants.reflection_resolution)));
